        ...
        public async Task<EvaluationResult> EvaluateSystemAsync(int topK = 10)
        {
            ...
            try
            {
                ...
                foreach (var query in queries)
                {
                    ...
                    {
                        Console.WriteLine(
                            $"Processing query {queryId}: '{queryText.Substring(0, Math.Min(50, queryText.Length))}...'");
                        ...
                        var retrievedDocs =
                            new List<(string Text, string DocumentId, string DocumentTitle, float Similarity)>();
                        ...
                        Console.WriteLine($"Retrieved document IDs: [{string.Join(", ", retrievedDocIds.Take(5))}]" +
                                          (retrievedDocIds.Count > 5 ? ", ..." : ""));
                        ...
                        Console.WriteLine(
                            $"  Retrieved {retrievedDocIds.Count} documents, {relevantRetrieved.Count} are relevant");
                        Console.WriteLine();
                    }
                    catch (Exception ex)
                    {
