...
namespace rag_experiment.Services
{
    public class QueryPreprocessor : IQueryPreprocessor
    {
        ...
        public QueryPreprocessor(IConfiguration configuration, HttpClient httpClient,
            ILogger<QueryPreprocessor> logger = null)
        {
        ...
        public async Task<string> ProcessQueryAsync(string query)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger?.LogWarning(ex,
                    "Error calling OpenAI API for query preprocessing. Falling back to manual processing.");
        ...
        public async Task<string> ProcessQueryAsync(string query, string conversationHistory)
        {
            ...
            catch (Exception ex)
            {
                // Log the error
                _logger?.LogWarning(ex,
                    "Error calling OpenAI API for query preprocessing with history. Falling back to manual processing.");
        ...
        private async Task<string> ProcessQueryWithOpenAIAsync(string query)
        {
            var chatMessage = new ChatMessage
            {
                Messages = new List<Message>
                {
                    new Message
                    {
                        Role = "system",
                        Content =
                            "Given the following user query, extract the key concepts or subject matter that should be used for a semantic search. Remove question words (e.g., 'what,' 'how,' 'why'), unnecessary phrasing, and focus only on the core ideas or entities. Return the result as a concise phrase or set of keywords."
                    },
                    new Message
                    {
                        Role = "user",
            ...
            {
                _logger?.LogWarning(
                    "OpenAI returned empty response for query preprocessing. Falling back to original query.");
                return query;
            }
        ...
        private async Task<string> ProcessQueryWithOpenAIAndHistoryAsync(string query, string conversationHistory)
        {
            var chatMessage = new ChatMessage
            {
                Messages = new List<Message>
                {
                    new Message
                    {
                        Role = "system",
                        Content =
                            "You are an expert at understanding conversational context and reformulating search queries. " +
            ...
            {
                _logger?.LogWarning(
                    "OpenAI returned empty response for query preprocessing with history. Falling back to original query.");
                return query;
            }

            _logger?.LogInformation(
                "Query reformulated with conversation context: '{OriginalQuery}' -> '{ProcessedQuery}'", query,
                processedQuery);
        ...
        public string ApplyQueryPatterns(string query)
        {
            ...
            var searchCommandPattern =
                new Regex(@"^(find|search for|look up|get|show me)\s+(.+)$", RegexOptions.IgnoreCase);
